...
class Histogram1D:
    ...
    def __init__(self, bins: Union[int, np.ndarray, list], range: Optional[Tuple[float, float]] = None):
        ...
        if isinstance(bins, int):
            ...
            self.edges: np.ndarray = np.linspace(range[0], range[1], bins + 1)
        else:
            self.edges = np.asarray(bins)
            if not np.all(np.diff(self.edges) > 0):
                raise ValueError("Bin edges must be monotonically increasing")

        self.counts: np.ndarray = np.zeros(len(self.edges) - 1, dtype=np.float64)
        self.sum_w2: np.ndarray = np.zeros(len(self.edges) - 1, dtype=np.float64) # Sum of weights squared
        self.underflow: float = 0.0
